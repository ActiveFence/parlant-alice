...
from activefence_client_sdk.client import ActiveFenceClient as SDKClient, AnalysisContext
from activefence_client_sdk.models import EvaluateMessageResponse, Actions
import parlant.sdk as p
from .moderation_service import ActiveFenceNLPServiceWrapper


class ActiveFence:
    def __init__(self, api_key: Optional[str] = None, app_name: Optional[str] = None, blocked_message: Optional[str] = None):
        """
        ...
        self.blocked_message = blocked_message or os.getenv('ACTIVEFENCE_BLOCKED_MESSAGE', 'The generated message was blocked by guardrails.')
    ...
    async def check_message_compliance(self, ctx: p.LoadedContext, payload: Any, exc: Exception | None) -> p.EngineHookResult:
        ...
        if result.action == Actions.DETECT:
        ...
        elif result.action == Actions.BLOCK or result.action == Actions.MASK:
            ...
            await ctx.session_event_emitter.emit_message_event(correlation_id=ctx.correlator.correlation_id, data=p.MessageEventData(
                message=message,
    ...
    async def configure_container(self, container: p.Container) -> p.Container:
        ...
        wrapped_nlp_service = ActiveFenceNLPServiceWrapper(
            original_nlp_service, 
            self._client, 
            logger
        )
